...
The Autonomy system integrates with several external APIs to provide enhanced
functionality. This reference covers all external APIs, their purposes, and
integration details.
...
**Note**: All API integrations are optional except Space-Track (required for
Starlink tracking). The system gracefully degrades functionality when APIs are
unavailable.
