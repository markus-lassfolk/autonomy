...
echo "=== Building RUTOS Autonomy Package with Correct Format ==="
...
# Create IPK package in correct RUTOS format
...
tar -czf "${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_correct.ipk" debian-binary control.tar.gz data.tar.gz control+data.sig

# Move IPK to project root
mv "${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_correct.ipk" "$PROJECT_ROOT/"

echo "IPK package created successfully!"
echo "Package: ${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_correct.ipk"

# Show package size
PACKAGE_SIZE=$(du -h "${PROJECT_ROOT}/${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_correct.ipk" | cut -f1)
echo "Package size: $PACKAGE_SIZE"

# Clean up temporary files
rm -f data.tar.gz control.tar.gz debian-binary control+data.sig
rm -rf CONTROL
...
echo ""
echo "This package uses the correct RUTOS format with signature file!"
echo "Install it using: opkg install ${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_correct.ipk"
