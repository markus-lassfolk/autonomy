# Simple VUCI Package Deployer
# Uses the already built packages from build-unique-vuci-packages.sh

$DeviceIP = "192.168.80.1"
$DeviceUser = "root"
$SSHKey = "C:\Users\markusla\OneDrive\IT\RUTOS Keys\rusos_private_key_openssh"

Write-Host "=== Deploying VUCI Packages ===" -ForegroundColor Green
Write-Host "Using pre-built packages with unique identifiers" -ForegroundColor Cyan
Write-Host ""

# Step 1: Find the built packages
Write-Host "Step 1: Locating built packages..." -ForegroundColor Yellow
$apiPackage = Get-ChildItem -Name "vuci-app-example-api_1.0-1_*.ipk" | Select-Object -First 1
$uiPackage = Get-ChildItem -Name "vuci-app-example-ui_1.0-1_*.ipk" | Select-Object -First 1
...
if ($apiPackage -and $uiPackage) {
    ...
    Write-Host "Step 2: Verifying package integrity..." -ForegroundColor Yellow
    ...
    wsl bash -c "file '$apiPackage' | grep -q 'Debian binary package'"
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ API package is valid Debian binary package" -ForegroundColor Green
    } else {
        Write-Host "❌ ERROR: API package is not a valid Debian binary package" -ForegroundColor Red
        exit 1
    }
    
    wsl bash -c "file '$uiPackage' | grep -q 'Debian binary package'"
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ UI package is valid Debian binary package" -ForegroundColor Green
    } else {
        Write-Host "❌ ERROR: UI package is not a valid Debian binary package" -ForegroundColor Red
        exit 1
    }
    
    # Step 3: Transfer packages to device
    Write-Host "Step 3: Transferring packages to device..." -ForegroundColor Yellow
    ...
    # Step 4: Verify packages on device
    Write-Host "Step 4: Verifying packages on device..." -ForegroundColor Yellow
    ...
    # Step 5: Install API package
    Write-Host "Step 5: Installing API package..." -ForegroundColor Yellow
    ...
    # Step 6: Install UI package
    Write-Host "Step 6: Installing UI package..." -ForegroundColor Yellow
    ...
    Write-Host "Step 8: Restarting services..." -ForegroundColor Yellow
    ...
    Write-Host "Step 9: Testing API functionality..." -ForegroundColor Yellow
    ...
    Write-Host "Step 10: Testing web interface..." -ForegroundColor Yellow
    ...
    # Step 11: Check menu configuration
    Write-Host "Step 11: Verifying menu configuration..." -ForegroundColor Yellow
    ...
    # Step 12: Check for UI assets
    Write-Host "Step 12: Checking for UI assets..." -ForegroundColor Yellow
    ...
    Write-Host "Please run build-unique-vuci-packages.sh first" -ForegroundColor Yellow
    exit 1
}
