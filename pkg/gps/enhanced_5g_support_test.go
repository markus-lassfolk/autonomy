...
func TestCollect5GNetworkInfo_Enabled(t *testing.T) {
	...
	if err != nil {
		...
		assert.NotZero(t, result.CollectedAt)
		assert.False(t, result.Valid) // Should be false without real data
	}
}
...
func TestEnhanced5GCellInfo_Structure(t *testing.T) {
	...
	assert.Equal(t, 1234, cellInfo.PCI)
...
func TestEnhanced5GNetworkInfo_Structure(t *testing.T) {
	...
	assert.Equal(t, "AT&T", networkInfo.NetworkOperator)
