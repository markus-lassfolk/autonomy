...
echo "=== Building RUTOS Autonomy Package (Minimal - No LuCI Dependencies) ==="
...
# Create IPK package with minimal dependencies
...
tar -czf "${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_minimal.ipk" debian-binary control.tar.gz data.tar.gz control+data.sig

# Move IPK to project root
mv "${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_minimal.ipk" "$PROJECT_ROOT/"

echo "IPK package created successfully!"
echo "Package: ${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_minimal.ipk"

# Show package size
PACKAGE_SIZE=$(du -h "${PROJECT_ROOT}/${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_minimal.ipk" | cut -f1)
...
echo ""
echo "This package uses minimal dependencies and should install successfully!"
echo "Install it using: opkg install ${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_minimal.ipk"
