...
$BuildDir = Join-Path $ScriptDir "build-openwrt"
$PackageName = "autonomy"
$LuciPackageName = "luci-app-autonomy"

Write-Status "Starting OpenWrt package build..."
...
# Create control file for main package (OpenWrt compatible)
...
Package: $PackageName
Version: $Version
Depends: luci-app-autonomy, uci, mwan3, ubus, gpsd, modemmanager
Architecture: $Architecture
Installed-Size: 20480
Description: Autonomous networking system for OpenWrt devices
 ...
 Note: GPS and cellular features require additional hardware support.
"@

$ControlContent | Out-File -FilePath (Join-Path $ControlDir "control") -Encoding UTF8
Write-Success "Created control file"

# Create postinst script (OpenWrt compatible)
Write-Status "Creating postinst script..."
$PostinstContent = @"
#!/bin/sh
# Post-installation script for autonomy package (OpenWrt)
...
echo "Note: GPS and cellular features may require additional configuration"
"@
...
Description: LuCI web interface for autonomy (OpenWrt)
...
$DataDir = Join-Path $BuildDir "temp_data_main"
...
$TempZipPath = Join-Path $BuildDir "temp_data.zip"
...
$TempZipPath = Join-Path $BuildDir "temp_control.zip"
...
$TempZipPath = Join-Path $BuildDir "temp_main.zip"
...
$LuciDataDir = Join-Path $BuildDir "temp_data_luci"
...
$TempZipPath = Join-Path $BuildDir "temp_luci_data.zip"
...
$TempZipPath = Join-Path $BuildDir "temp_luci_control.zip"
...
$TempZipPath = Join-Path $BuildDir "temp_luci.zip"
...
$InstructionsContent = @"
# OpenWrt Autonomy Package Installation
...
1. Open your OpenWrt web interface
...
1. Copy the IPK files to your OpenWrt device
2. Install via SSH:
   ```bash
...
2. Insert USB drive into your OpenWrt device
3. Use the web interface to install from USB

## OpenWrt-Specific Notes:
- GPS features require gpsd package: `opkg install gpsd`
- Cellular features require modemmanager: `opkg install modemmanager`
- Some hardware-specific features may need additional configuration
- Check OpenWrt documentation for your specific device
...
Get-ChildItem -Path $BuildDir -Filter "temp_*.zip" | Remove-Item -Force
Get-ChildItem -Path $BuildDir -Filter "temp_*" -Directory | Remove-Item -Recurse -Force
