...
echo "=== Building RUTOS Autonomy Package with Fixed Structure ==="
...
# Create IPK package with correct structure
...
# Create control file (directly, not in CONTROL subdirectory)
...
# Create postinst script (directly, not in CONTROL subdirectory)
...
# Create prerm script (directly, not in CONTROL subdirectory)
...
# Create control.tar.gz with files directly in root (not in CONTROL subdirectory)
...
tar -czf "${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_fixed.ipk" debian-binary control.tar.gz data.tar.gz control+data.sig

# Move IPK to project root
mv "${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_fixed.ipk" "$PROJECT_ROOT/"

echo "IPK package created successfully!"
echo "Package: ${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_fixed.ipk"

# Show package size
PACKAGE_SIZE=$(du -h "${PROJECT_ROOT}/${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_fixed.ipk" | cut -f1)
...
echo ""
echo "This package uses the correct structure matching working RUTOS packages!"
echo "Install it using: opkg install ${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_fixed.ipk"
