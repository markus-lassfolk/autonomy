...
# Create data.tar.gz for main package
...
# Move all files except CONTROL to data directory
Get-ChildItem -Path $AutonomyPackageDir -Exclude "CONTROL" | ForEach-Object {
    Move-Item $_.FullName $DataDir
}

# Create tar.gz (using PowerShell compression)
$TempTarPath = Join-Path $BuildDir "temp.tar"
tar -cf $TempTarPath -C $DataDir .
gzip -f $TempTarPath
Move-Item "$TempTarPath.gz" $DataTarPath
...
$TempTarPath = Join-Path $BuildDir "temp.tar"
tar -cf $TempTarPath -C $ControlDir .
gzip -f $TempTarPath
Move-Item "$TempTarPath.gz" $ControlTarPath
...
$TempTarPath = Join-Path $BuildDir "temp.tar"
tar -cf $TempTarPath -C $AutonomyPackageDir .
gzip -f $TempTarPath
Move-Item "$TempTarPath.gz" $IpkPath
...
Get-ChildItem -Path $LuciPackageDir -Exclude "CONTROL" | ForEach-Object {
    Move-Item $_.FullName $LuciDataDir
}

# Create tar.gz
$TempTarPath = Join-Path $BuildDir "temp.tar"
tar -cf $TempTarPath -C $LuciDataDir .
gzip -f $TempTarPath
Move-Item "$TempTarPath.gz" $LuciDataTarPath
...
$TempTarPath = Join-Path $BuildDir "temp.tar"
tar -cf $TempTarPath -C $LuciControlDir .
gzip -f $TempTarPath
Move-Item "$TempTarPath.gz" $LuciControlTarPath
...
$TempTarPath = Join-Path $BuildDir "temp.tar"
tar -cf $TempTarPath -C $LuciPackageDir .
gzip -f $TempTarPath
Move-Item "$TempTarPath.gz" $LuciIpkPath
...
if (Test-Path (Join-Path $BuildDir "temp.tar")) {
    Remove-Item (Join-Path $BuildDir "temp.tar") -Force
}
...
Write-Status "  Main package: $([math]::Round($MainPackageSize, 2)) MB"
Write-Status "  LuCI package: $([math]::Round($LuciPackageSize, 2)) MB"
