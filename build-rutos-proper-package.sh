...
echo "=== Building Proper RUTOS Autonomy Package for Package Manager ==="
...
tar -czf "${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_proper.ipk" debian-binary control.tar.gz data.tar.gz

# Move IPK to project root
mv "${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_proper.ipk" "$PROJECT_ROOT/"

echo "IPK package created successfully!"
echo "Package: ${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_proper.ipk"

# Show package size
PACKAGE_SIZE=$(du -h "${PROJECT_ROOT}/${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_proper.ipk" | cut -f1)
...
echo ""
echo "This package should now be recognized by the RUTOS Package Manager!"
echo "Install it using: opkg install ${PACKAGE_NAME}_${VERSION}_${ARCHITECTURE}_proper.ipk"
