...
echo "=== Fixing Example API on RUTOS Device ==="
...
TEMP_SSH_KEY="/tmp/rutos_fix_key_$$"
cp "$SSH_KEY" "$TEMP_SSH_KEY"
chmod 600 "$TEMP_SSH_KEY"
SSH_OPTS="-i $TEMP_SSH_KEY"

echo "Installing example API files..."

# Create the example API Lua file
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "cat > /tmp/example.lua" << 'EOF'
...
# Copy the file to the correct location and make it executable
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "cp /tmp/example.lua /usr/libexec/rpcd/example.lua && chmod +x /usr/libexec/rpcd/example.lua"

# Create VUCI menu configuration
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "mkdir -p /tmp/vuci-menu"

ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "cat > /tmp/vuci-menu/example.json" << 'EOF'
{
    "name": "Example",
    "icon": "icon-example",
    "path": "example",
...
# Copy menu file to correct location
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "cp /tmp/vuci-menu/example.json /usr/share/vuci/menu.d/example.json"

# Create example UI directory and files
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "mkdir -p /tmp/example-ui"

# Create the example UI HTML file
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "cat > /tmp/example-ui/index.html" << 'EOF'
<!DOCTYPE html>
<html>
<head>
...
# Copy UI files to correct location
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "mkdir -p /www/views/example && cp /tmp/example-ui/index.html /www/views/example/"

# Restart services
echo "Restarting services..."
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "/etc/init.d/rpcd restart && /etc/init.d/uhttpd restart"
...
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "ubus list | grep example"

# Test API calls
echo "Testing API calls..."
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "ubus call example status"
ssh $SSH_OPTS -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$DEVICE_USER@$DEVICE_IP" "ubus call example info"
...
echo ""
echo "You can now:"
echo "1. Access the UI at: http://$DEVICE_IP/example/"
echo "2. Test the API: ssh $DEVICE_USER@$DEVICE_IP 'ubus call example status'"
