...
int gps_error_recovery_init(void) {
    if (g_error_recovery_initialized) {
        printf("WARN: "GPS error recovery already initialized"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    printf("INFO: "GPS error recovery initialized successfully"\n"\n"\n"\n"\n"\n"\n"\n");
    return AUTONOMY_SUCCESS;
}
...
int gps_error_recovery_record_error(int source_id, gps_error_type_t error_type, int error_code, const char *error_message) {
    ...
    printf("WARN: "GPS error recorded for source %d: %s (code: %d) - %s", 
...
void update_source_error_tracking(int source_id, gps_error_type_t error_type) {
    ...
    if (should_retry_error(source, error_type)) {
        ...
        printf("DEBUG: "Source %d retry %d/%d, backoff until %ld", 
...
void update_source_status(gps_source_error_local_t *source) {
    if (source->error_rate > g_error_recovery.error_threshold_ratio) {
        if (source->status == SOURCE_STATUS_ACTIVE) {
            ...
            printf("WARN: "Source %d status changed to DEGRADED (error rate: %.2f%%)", 
        ...
        if (source->status != SOURCE_STATUS_ACTIVE) {
            ...
            printf("INFO: "Source %d status restored to ACTIVE (error rate: %.2f%%)", 
                     source->source_id, source->error_rate * 100.0\n"\n"\n"\n"\n"\n"\n"\n");
        }
    }
    ...
    if (source->current_retry_count >= g_error_recovery.max_retry_attempts) {
        ...
        printf("ERROR: "Source %d status changed to FAILED (max retries exceeded)", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    }
}
...
static bool perform_retry_recovery(gps_source_error_local_t *source, gps_error_type_t error_type) {
    printf("DEBUG: "Attempting retry recovery for source %d (attempt %d/%d)", 
    ...
    switch (source->source_id) {
        ...
        default:
            printf("WARN: "Unknown GPS source type for retry: %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (success) {
        ...
        source->current_retry_count = 0;
        source->backoff_until = 0;
        printf("INFO: "Retry recovery successful for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    } else {
        printf("WARN: "Retry recovery failed for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    }
...
static bool perform_fallback_recovery(gps_source_error_local_t *source, gps_error_type_t error_type) {
    printf("DEBUG: "Attempting fallback recovery for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    switch (source->source_id) {
        ...
        default:
            printf("WARN: "Unknown GPS source type for fallback: %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
            success = false; // Use configurable setting // Use configurable setting
            break;
    }
    ...
    if (success) {
        source->recovered_errors++;
        printf("INFO: "Fallback recovery successful for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    } else {
        printf("WARN: "Fallback recovery failed for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    }
...
static bool perform_reset_recovery(gps_source_error_local_t *source, gps_error_type_t error_type) {
    printf("DEBUG: "Attempting reset recovery for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    switch (source->source_id) {
        ...
        default:
            printf("WARN: "Unknown GPS source type for reset: %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
            success = false; // Use configurable setting // Use configurable setting
            break;
    }
    ...
    if (success) {
        ...
        source->backoff_until = 0;
        source->status = SOURCE_STATUS_ACTIVE;
        printf("INFO: "Reset recovery successful for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    } else {
        printf("WARN: "Reset recovery failed for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    }
...
static bool perform_degrade_recovery(gps_source_error_local_t *source, gps_error_type_t error_type) {
    printf("DEBUG: "Attempting degrade recovery for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    switch (source->source_id) {
        ...
        default:
            printf("WARN: "Unknown GPS source type for degrade: %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
            success = false; // Use configurable setting // Use configurable setting
            break;
    }
    ...
    if (success) {
        source->recovered_errors++;
        source->status = SOURCE_STATUS_DEGRADED;
        printf("INFO: "Degrade recovery successful for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    } else {
        printf("WARN: "Degrade recovery failed for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    }
...
static bool perform_switch_source_recovery(gps_source_error_local_t *source, gps_error_type_t error_type) {
    printf("DEBUG: "Attempting switch source recovery for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    switch (source->source_id) {
        case GPS_SOURCE_TYPE_RUTOS:
            ...
            printf("DEBUG: "No alternative GPS source available for RUTOS"\n"\n"\n"\n"\n"\n"\n"\n");
            success = false; // Use configurable setting // Use configurable setting
            break;
        ...
        case GPS_SOURCE_TYPE_STARLINK:
            ...
            if (gps_rutos_is_initialized()) {
                ...
                printf("INFO: "Switching from Starlink to RUTOS GPS"\n"\n"\n"\n"\n"\n"\n"\n");
            } else {
                success = false; // Use configurable setting // Use configurable setting
                printf("WARN: "RUTOS GPS not available for fallback"\n"\n"\n"\n"\n"\n"\n"\n");
            }
            break;
        ...
        case GPS_SOURCE_TYPE_EXTERNAL:
            ...
            if (gps_rutos_is_initialized()) {
                success = true; // Use configurable setting // Use configurable setting
                printf("INFO: "Switching from external to RUTOS GPS"\n"\n"\n"\n"\n"\n"\n"\n");
            } else {
                success = false; // Use configurable setting // Use configurable setting
                printf("WARN: "RUTOS GPS not available for fallback"\n"\n"\n"\n"\n"\n"\n"\n");
            }
        ...
        default:
            printf("WARN: "Unknown GPS source type for switch: %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
            success = false; // Use configurable setting // Use configurable setting
            break;
    }
    ...
    if (success) {
        source->recovered_errors++;
        source->status = SOURCE_STATUS_ACTIVE;
        printf("INFO: "Switch source recovery successful for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    } else {
        printf("WARN: "Switch source recovery failed for source %d", source->source_id\n"\n"\n"\n"\n"\n"\n"\n");
    }
...
int gps_error_recovery_set_config(const gps_error_recovery_config_t *config) {
    ...
    printf("INFO: "GPS error recovery configuration updated"\n"\n"\n"\n"\n"\n"\n"\n");
    return AUTONOMY_SUCCESS;
}
...
int gps_error_recovery_set_enabled(bool enabled) {
    ...
    printf("INFO: "GPS error recovery %s", enabled ? "enabled" : "disabled"\n"\n"\n"\n"\n"\n"\n"\n");
    return AUTONOMY_SUCCESS;
}
...
int gps_error_recovery_force_recovery(int source_id) {
    ...
    printf("INFO: "Forced error recovery for source %d", source_id\n"\n"\n"\n"\n"\n"\n"\n");
    return AUTONOMY_SUCCESS;
}
...
int gps_error_recovery_reset(void) {
    ...
    printf("INFO: "GPS error recovery reset"\n"\n"\n"\n"\n"\n"\n"\n");
    return AUTONOMY_SUCCESS;
}
...
void gps_error_recovery_cleanup(void) {
    ...
    printf("INFO: "GPS error recovery cleaned up"\n"\n"\n"\n"\n"\n"\n"\n");
}

// Starlink GPS recovery helper functions
static bool starlink_gps_retry_recovery(void) {
    printf("DEBUG: "Attempting Starlink GPS retry recovery"\n"\n"\n"\n"\n"\n"\n"\n");
    
    // Try to restart Starlink GPS service
    int ret = system("systemctl restart starlink-gps 2>/dev/null"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "Failed to restart Starlink GPS service"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    if (ret != 0) {
        printf("WARN: "Starlink GPS service not active after restart"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    if (!ctx) {
        printf("WARN: "Failed to connect to UBUS for Starlink GPS check"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    if (ret != 0) {
        printf("WARN: "Starlink GPS UBUS service not found"\n"\n"\n"\n"\n"\n"\n"\n");
        ubus_free(ctx\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    if (ret == 0) {
        printf("INFO: "Starlink GPS retry recovery successful"\n"\n"\n"\n"\n"\n"\n"\n");
        return true;
    } else {
        printf("WARN: "Starlink GPS UBUS call failed"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
}

static bool starlink_gps_fallback_recovery(void) {
    printf("DEBUG: "Attempting Starlink GPS fallback recovery"\n"\n"\n"\n"\n"\n"\n"\n");
    
    // Try to reinitialize Starlink GPS system
    int ret = system("systemctl stop starlink-gps 2>/dev/null"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "Failed to stop Starlink GPS service"\n"\n"\n"\n"\n"\n"\n"\n");
    }
    ...
    if (ret != 0) {
        printf("WARN: "Failed to start Starlink GPS service"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    ret = system("systemctl is-active starlink-gps > /dev/null 2>&1"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "Starlink GPS service not active after fallback"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    
    printf("INFO: "Starlink GPS fallback recovery successful"\n"\n"\n"\n"\n"\n"\n"\n");
    return true;
}

static bool starlink_gps_reset_recovery(void) {
    printf("DEBUG: "Attempting Starlink GPS reset recovery"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (ret != 0) {
        printf("WARN: "Failed to start Starlink tracker"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    ret = system("systemctl is-active starlink-gps > /dev/null 2>&1"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "Starlink GPS service not active after reset"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    
    printf("INFO: "Starlink GPS reset recovery successful"\n"\n"\n"\n"\n"\n"\n"\n");
    return true;
}

static bool starlink_gps_degrade_recovery(void) {
    printf("DEBUG: "Attempting Starlink GPS degrade recovery"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    int ret = system("systemctl restart starlink-gps 2>/dev/null"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "Failed to restart Starlink GPS with degraded settings"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    ret = system("systemctl is-active starlink-gps > /dev/null 2>&1"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "Starlink GPS service not active in degraded mode"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    
    printf("INFO: "Starlink GPS degrade recovery successful"\n"\n"\n"\n"\n"\n"\n"\n");
    return true;
}

// External GPS recovery helper functions
static bool external_gps_retry_recovery(void) {
    printf("DEBUG: "Attempting external GPS retry recovery"\n"\n"\n"\n"\n"\n"\n"\n");
    
    // Try to restart external GPS service
    int ret = system("systemctl restart external-gps 2>/dev/null"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "Failed to restart external GPS service"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    if (ret != 0) {
        printf("WARN: "External GPS service not active after restart"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    struct ubus_context* ctx = ubus_connect(NULL\n"\n"\n"\n"\n"\n"\n"\n");
    if (!ctx) {
        printf("WARN: "Failed to connect to UBUS for external GPS check"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    if (ret != 0) {
        printf("WARN: "External GPS UBUS service not found"\n"\n"\n"\n"\n"\n"\n"\n");
        ubus_free(ctx\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    if (ret == 0) {
        printf("INFO: "External GPS retry recovery successful"\n"\n"\n"\n"\n"\n"\n"\n");
        return true;
    } else {
        printf("WARN: "External GPS UBUS call failed"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
}

static bool external_gps_fallback_recovery(void) {
    printf("DEBUG: "Attempting external GPS fallback recovery"\n"\n"\n"\n"\n"\n"\n"\n");
    
    // Try to reinitialize external GPS system
    int ret = system("systemctl stop external-gps 2>/dev/null"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "Failed to stop external GPS service"\n"\n"\n"\n"\n"\n"\n"\n");
    }
    ...
    if (ret != 0) {
        printf("WARN: "Failed to start external GPS service"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    ret = system("systemctl is-active external-gps > /dev/null 2>&1"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "External GPS service not active after fallback"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    
    printf("INFO: "External GPS fallback recovery successful"\n"\n"\n"\n"\n"\n"\n"\n");
    return true;
}

static bool external_gps_reset_recovery(void) {
    printf("DEBUG: "Attempting external GPS reset recovery"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    ret = system("systemctl is-active external-gps > /dev/null 2>&1"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "External GPS service not active after reset"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    
    printf("INFO: "External GPS reset recovery successful"\n"\n"\n"\n"\n"\n"\n"\n");
    return true;
}

static bool external_gps_degrade_recovery(void) {
    printf("DEBUG: "Attempting external GPS degrade recovery"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    int ret = system("systemctl restart external-gps 2>/dev/null"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "Failed to restart external GPS with degraded settings"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    ...
    ret = system("systemctl is-active external-gps > /dev/null 2>&1"\n"\n"\n"\n"\n"\n"\n"\n");
    if (ret != 0) {
        printf("WARN: "External GPS service not active in degraded mode"\n"\n"\n"\n"\n"\n"\n"\n");
        return false;
    }
    
    printf("INFO: "External GPS degrade recovery successful"\n"\n"\n"\n"\n"\n"\n"\n");
    return true;
}
