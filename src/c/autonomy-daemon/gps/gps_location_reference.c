...
int gps_location_reference_init(const gps_location_reference_config_t* config) {
    if (g_location_ref_initialized) {
        printf("WARN: "GPS location reference already initialized"\n"\n"\n"\n"\n"\n"\n"\n");
        return AUTONOMY_SUCCESS;
    }
    
    if (!config) {
        printf("ERROR: "GPS location reference config is NULL"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (pthread_mutex_init(&g_location_ref_manager.mutex, NULL) != 0) {
        printf("ERROR: "Failed to initialize GPS location reference mutex"\n"\n"\n"\n"\n"\n"\n"\n");
        return AUTONOMY_ERROR_SYSTEM;
    }
    
    // Initialize database
    if (init_location_database() != AUTONOMY_SUCCESS) {
        printf("ERROR: "Failed to initialize location reference database"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (!g_location_ref_manager.location_cache) {
        printf("ERROR: "Failed to allocate memory for location cache"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (config->enabled) {
        ...
        if (pthread_create(&g_location_ref_manager.cleanup_thread, NULL, 
            ...
            printf("ERROR: "Failed to create location reference cleanup thread"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    printf("INFO: "GPS location reference system initialized",
...
void gps_location_reference_cleanup(void) {
    ...
    printf("INFO: "GPS location reference system cleaned up"\n"\n"\n"\n"\n"\n"\n"\n");
}
...
static void* cleanup_thread_worker(void* arg) {
    ...
    printf("INFO: "GPS location reference cleanup thread started",
    ...
    while (g_location_ref_initialized && g_location_ref_manager.thread_running) {
        ...
        if (cleaned_up > 0) {
            printf("INFO: "GPS location reference cleanup completed",
                     "locations_cleaned", cleaned_up\n"\n"\n"\n"\n"\n"\n"\n");
        }
    }
    
    printf("INFO: "GPS location reference cleanup thread stopped"\n"\n"\n"\n"\n"\n"\n"\n");
    return NULL;
}
...
static int init_location_database(void) {
    ...
    if (last_slash) {
        *last_slash = '\0';
        if (mkdir(db_dir, 0755) != 0 && errno != EEXIST) {
            printf("ERROR: "Failed to create database directory", "path", db_dir\n"\n"\n"\n"\n"\n"\n"\n");
            return AUTONOMY_ERROR_SYSTEM;
        }
    }
    ...
    if (result != SQLITE_OK) {
        printf("ERROR: "Failed to open location reference database",
                  "path", db_path,
    ...
    if (result != SQLITE_OK) {
        printf("ERROR: "Failed to create location reference schema", "error", error_msg\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    printf("INFO: "GPS location reference database initialized", "path", db_path\n"\n"\n"\n"\n"\n"\n"\n");
...
int gps_location_reference_get_or_create(double latitude, double longitude, 
    ...
    if (g_location_ref_manager.last_location_valid) {
        ...
        if (distance < g_location_ref_manager.config.movement_threshold_meters) {
            ...
            *location_id = g_location_ref_manager.last_location.location_id;
            ...
            printf("DEBUG: "Using existing location reference",
    ...
    if (create_new_location) {
        ...
        if (find_nearby_location_reference(latitude, longitude, location_id) == AUTONOMY_SUCCESS) {
            printf("DEBUG: "Found nearby location reference",
                      "location_id", *location_id\n"\n"\n"\n"\n"\n"\n"\n");
            create_new_location = false;
        }
    }
    ...
    if (create_new_location) {
        ...
        if (create_new_location_reference(latitude, longitude, accuracy, gps_source, location_id) == AUTONOMY_SUCCESS) {
            ...
            printf("INFO: "Created new location reference",
                     "location_id", *location_id,
                     "lat", latitude,
                     "lon", longitude,
                     "accuracy", accuracy\n"\n"\n"\n"\n"\n"\n"\n");
...
int gps_location_reference_force_cleanup(void) {
    ...
    printf("INFO: "Location reference cleanup completed", 
    ...
    if (sqlite3_prepare_v2(g_location_ref_manager.db, sql, -1, &stmt, NULL) == SQLITE_OK) {
        *location_id = g_location_ref_manager.next_location_id++;
        ...
        if (sqlite3_step(stmt) == SQLITE_DONE) {
            ...
            printf("DEBUG: "Created new location reference", 
                          "location_id", *location_id,
                          "latitude", latitude,
                          "longitude", longitude,
                          "accuracy", accuracy,
