...
// Cell tower information for intelligent caching
typedef struct {
    char cell_id[32];
    int rsrp;                    // Signal strength
    int rsrq;                    // Signal quality
    int earfcn;                  // Frequency
    int pci;                     // Physical Cell ID
    char type[16];               // "intra" or "inter"
} cell_tower_info_t;

// Cell environment for change detection
typedef struct {
    cell_tower_info_t serving_cell;
    cell_tower_info_t neighbor_cells[16];  // Up to 16 neighbor cells
    int neighbor_count;
    time_t timestamp;
    char location_hash[64];      // Hash of the cellular environment
} cell_environment_t;

// Intelligent cache configuration
typedef struct {
    time_t max_cache_age;        // Fallback cache time (e.g., 1 hour)
    time_t debounce_delay;       // Debounce delay (e.g., 10 seconds)
    double tower_change_threshold; // Percentage threshold for tower changes (e.g., 0.35 = 35%)
    int top_towers_count;        // Number of top towers to monitor (e.g., 5)
} intelligent_cache_config_t;

// Intelligent cache state
typedef struct {
    cell_environment_t *last_environment;
    time_t last_location_query;
    opencellid_response_t *last_location_result;
    time_t debounce_timer;
    intelligent_cache_config_t config;
 ...
 * @param env Current cell environment
 * @return true if query should be made, false to use cache
 */
bool intelligent_cache_should_query_new_location(const cell_environment_t* env);

/**
 * Update cache with new location result
 * @param env Cell environment that triggered the query
 * @param result Location result from API
 */
 ...
 * @param result Cached location result
 * @return true if valid cache available, false otherwise
 */
 ...
 * @return Percentage of towers that changed (0.0-1.0)
 */
 ...
 * @param current Current environment
 * @param previous Previous environment
 * @return true if top towers changed, false otherwise
 */
 ...
 * @param env Cell environment
 * @param hash Buffer to store hash (must be at least 64 bytes)
 */
