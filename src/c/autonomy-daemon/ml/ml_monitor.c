...
ml_persistent_state_t* ml_monitor_init_storage(const char *filepath, size_t *storage_size) {
    ...
    printf("INFO: "Initializing ML monitor storage: %s", filepath\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (last_slash) {
        *last_slash = '\0';
        ...
        if (mkdir(dir_path, 0755) < 0 && errno != EEXIST) {
            printf("ERROR: "Failed to create storage directory: %s", strerror(errno)\n"\n"\n"\n"\n"\n"\n"\n");
            return NULL;
        }
    ...
    if (fd < 0) {
        printf("ERROR: "Failed to open storage file: %s", strerror(errno)\n"\n"\n"\n"\n"\n"\n"\n");
        return NULL;
    }
    ...
    *storage_size = sizeof(ml_persistent_state_t\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (ftruncate(fd, *storage_size) < 0) {
        printf("ERROR: "Failed to resize storage file: %s", strerror(errno)\n"\n"\n"\n"\n"\n"\n"\n");
        close(fd\n"\n"\n"\n"\n"\n"\n"\n");
        return NULL;
    }
    ...
    if (state == MAP_FAILED) {
        printf("ERROR: "Failed to memory map storage file: %s", strerror(errno)\n"\n"\n"\n"\n"\n"\n"\n");
        return NULL;
    }
...
ml_monitor_t* ml_monitor_init(const ml_monitor_config_t *config) {
    ...
    if (!monitor->state) {
        printf("ERROR: "Failed to initialize ML storage"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (analytics_result != ML_MONITOR_SUCCESS) {
        printf("ERROR: "Failed to initialize ML analytics: %d", analytics_result\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (pthread_mutex_init(&monitor->state_mutex, NULL) != 0) {
        printf("ERROR: "Failed to initialize ML monitor mutex"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    printf("INFO: "ML monitor initialized successfully (memory usage: %zu KB)", 
...
void ml_monitor_cleanup(ml_monitor_t *monitor) {
    ...
    printf("INFO: "Cleaning up ML monitor"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    free(monitor\n"\n"\n"\n"\n"\n"\n"\n");
    printf("INFO: "ML monitor cleanup completed"\n"\n"\n"\n"\n"\n"\n"\n");
}
...
int ml_monitor_sync_storage(ml_monitor_t *monitor) {
    ...
    if (msync(monitor->state, monitor->storage_size, MS_ASYNC) < 0) {
        printf("ERROR: "Failed to sync ML storage: %s", strerror(errno)\n"\n"\n"\n"\n"\n"\n"\n");
...
int ml_monitor_add_observation(ml_monitor_t *monitor, const ml_observation_t *observation) {
    ...
    printf("DEBUG: "Added ML observation (total: %u, SNR: %.2f, obstruction: %u%%)", 
              state->total_observations,
...
int ml_monitor_update_sky_grid(ml_monitor_t *monitor, const ml_observation_t *observation) {
    ...
    if (ml_monitor_location_changed_threshold(grid->learned_lat_e7, grid->learned_lon_e7,
        ...
        printf("INFO: "Location changed significantly, soft-resetting sky grid"\n"\n"\n"\n"\n"\n"\n"\n");
...
static void* ml_monitor_collection_thread(void *arg) {
    ...
    printf("INFO: "ML monitor collection thread stopped after %d collections", collection_count\n"\n"\n"\n"\n"\n"\n"\n");
    return NULL;
}
...
int ml_monitor_stop(ml_monitor_t *monitor) {
    ...
    printf("INFO: "Stopping ML monitor"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    printf("INFO: "ML monitor stopped"\n"\n"\n"\n"\n"\n"\n"\n");
...
int ml_monitor_trigger_proactive_optimization(ml_monitor_t *monitor, uint8_t probability, uint8_t confidence) {
    ...
    printf("INFO: "Triggering proactive optimization - probability: %d, confidence: %d", probability, confidence\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (probability >= 200 && confidence >= 180) {
        ...
        printf("INFO: "Proactive failover triggered"\n"\n"\n"\n"\n"\n"\n"\n");
