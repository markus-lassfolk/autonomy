...
static bool mock_starlink_available = true;
static bool mock_gps_available = true;
static bool mock_weather_available = true;
...
int main() {
    ...
    config.enabled = true;
    config.collection_interval_seconds = 5; // Fast for testing
    ...
    obs.obstruction_pct = 8;
    obs.azimuth_deg = 225;
    obs.elevation_deg = 55;
    obs.satellites_visible = 12;
    
    // GPS data
    obs.latitude_e7 = 595329444;   // Stockholm coordinates * 1e7
    obs.longitude_e7 = 180686111;  // Stockholm coordinates * 1e7
    obs.altitude_m = 25;
    obs.speed_kmh = 0; // Stationary
    
    // Weather data
    obs.temperature_c = 15;
    obs.humidity_pct = 65;
    obs.pressure_hpa = 1015;
    obs.wind_speed_ms = 3;
    ...
    moved_obs.latitude_e7 += 9000;  // ~100m north
    moved_obs.longitude_e7 += 9000; // ~100m east
    moved_obs.timestamp = time(NULL) + 300; // 5 minutes later
    ...
    for (int i = 0; i < 10; i++) {
        ml_observation_t mobile_obs = obs;
        mobile_obs.timestamp = time(NULL) + i * 30; // Every 30 seconds
        mobile_obs.latitude_e7 += i * 1000; // Moving north
    ...
    auto test_callback = [](uint8_t probability, uint8_t confidence, time_t when, void *user_data) {
