...
int mqtt_client_init(const mqtt_config_t* config) {
        ...
        if (uci_fp) {
            ...
            if (fgets(uci_host, sizeof(uci_host), uci_fp)) {
                ...
                if (strlen(uci_host) > 0) {
                    ...
                    printf("DEBUG: "Using UCI configured MQTT broker host", "host", uci_host\n"\n"\n"\n"\n"\n"\n"\n");
                } else {
                    strcpy(g_mqtt_client.config.broker_host, "localhost"\n"\n"\n"\n"\n"\n"\n"\n");
                    printf("DEBUG: "Using fallback MQTT broker host", "host", "localhost"\n"\n"\n"\n"\n"\n"\n"\n");
                }
            } else {
                strcpy(g_mqtt_client.config.broker_host, "localhost"\n"\n"\n"\n"\n"\n"\n"\n");
                printf("DEBUG: "Using fallback MQTT broker host", "host", "localhost"\n"\n"\n"\n"\n"\n"\n"\n");
        ...
        if (uci_port_fp) {
            ...
            if (fgets(uci_port, sizeof(uci_port), uci_port_fp)) {
                ...
                if (strlen(uci_port) > 0) {
                    ...
                    printf("DEBUG: "Using UCI configured MQTT broker port", "port", g_mqtt_client.config.broker_port\n"\n"\n"\n"\n"\n"\n"\n");
                } else {
                    ...
                    printf("DEBUG: "Using fallback MQTT broker port", "port", 1883\n"\n"\n"\n"\n"\n"\n"\n");
                }
            } else {
        ...
        if (uci_client_fp) {
            ...
            if (fgets(uci_client_id, sizeof(uci_client_id), uci_client_fp)) {
                ...
                if (strlen(uci_client_id) > 0) {
                    ...
                    printf("DEBUG: "Using UCI configured MQTT client ID", "client_id", uci_client_id\n"\n"\n"\n"\n"\n"\n"\n");
                } else {
                    strcpy(g_mqtt_client.config.client_id, "autonomy_daemon"\n"\n"\n"\n"\n"\n"\n"\n");
                    printf("DEBUG: "Using fallback MQTT client ID", "client_id", "autonomy_daemon"\n"\n"\n"\n"\n"\n"\n"\n");
                }
            } else {
                strcpy(g_mqtt_client.config.client_id, "autonomy_daemon"\n"\n"\n"\n"\n"\n"\n"\n");
                printf("DEBUG: "Using fallback MQTT client ID", "client_id", "autonomy_daemon"\n"\n"\n"\n"\n"\n"\n"\n");
    ...
    if (!g_mqtt_client.mutex) {
        printf("ERROR: "Failed to allocate memory for MQTT client mutex"\n"\n"\n"\n"\n"\n"\n"\n");
        return -1;
    }
...
static int mqtt_receive_packet(uint8_t* packet, int max_length) {
    ...
    if (select_result < 0) {
        printf("ERROR: "MQTT select error: %s", strerror(errno)\n"\n"\n"\n"\n"\n"\n"\n");
        return -1;
    ...
    if (received < 0) {
        ...
        printf("ERROR: "MQTT receive error: %s", strerror(errno)\n"\n"\n"\n"\n"\n"\n"\n");
        return -1;
    } else if (received == 0) {
        printf("WARN: "MQTT connection closed by server"\n"\n"\n"\n"\n"\n"\n"\n");
        return -1;
    }
    
    // Validate MQTT packet structure
    if (received < 2) {
        printf("WARN: "Received incomplete MQTT packet"\n"\n"\n"\n"\n"\n"\n"\n");
        return -1;
    }
    ...
    if (packet_type < 1 || packet_type > 14) {
        printf("WARN: "Invalid MQTT packet type: %d", packet_type\n"\n"\n"\n"\n"\n"\n"\n");
        return -1;
    }
    ...
    switch (packet_type) {
        case 1: // CONNECT
            if (packet_flags != 0) {
                printf("WARN: "Invalid CONNECT packet flags: %d", packet_flags\n"\n"\n"\n"\n"\n"\n"\n");
                return -1;
            }
            break;
        case 2: // CONNACK
            if (packet_flags != 0) {
                printf("WARN: "Invalid CONNACK packet flags: %d", packet_flags\n"\n"\n"\n"\n"\n"\n"\n");
                return -1;
            }
            break;
        case 3: // PUBLISH
        ...
        case 7: // PUBCOMP
            if (packet_flags != 0) {
                printf("WARN: "Invalid PUBACK/PUBREC/PUBREL/PUBCOMP packet flags: %d", packet_flags\n"\n"\n"\n"\n"\n"\n"\n");
                return -1;
            }
            break;
        case 8: // SUBSCRIBE
            if (packet_flags != 2) {
                printf("WARN: "Invalid SUBSCRIBE packet flags: %d", packet_flags\n"\n"\n"\n"\n"\n"\n"\n");
                return -1;
            }
            break;
        case 9: // SUBACK
        ...
        case 14: // DISCONNECT
            if (packet_flags != 0) {
                printf("WARN: "Invalid packet flags for type %d: %d", packet_type, packet_flags\n"\n"\n"\n"\n"\n"\n"\n");
                return -1;
            }
            break;
    }
    ...
    if (remaining_length < 0 || remaining_length > max_length - pos) {
        printf("WARN: "Invalid MQTT remaining length: %d", remaining_length\n"\n"\n"\n"\n"\n"\n"\n");
        return -1;
    }
    
    // Check if we have the complete packet
    if (pos + remaining_length > received) {
        printf("DEBUG: "Incomplete MQTT packet, need %d more bytes", 
                       pos + remaining_length - received\n"\n"\n"\n"\n"\n"\n"\n");
        return 0; // Need more data
    }
    
    printf("DEBUG: "Received MQTT packet: type=%d, flags=%d, length=%d", 
...
static int mqtt_parse_suback(const uint8_t* packet, int length) {
    ...
    for (int i = 0; i < return_code_count; i++) {
        ...
        // 0x80 is failure
        if (return_code == 0x80) {
            printf("WARN: "MQTT subscription failed for topic at position %d", i\n"\n"\n"\n"\n"\n"\n"\n");
            return -1;
        }
    }
