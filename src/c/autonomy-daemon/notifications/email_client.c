...
// Callback for curl to read email data
static size_t email_read_callback(void* ptr, size_t size, size_t nmemb, email_upload_ctx_t* ctx) {
...
static void parse_recipients(email_client_t* client, const char* recipients_str) {
    if (!client || !recipients_str) return;
...
int email_client_init(email_client_t* client, const email_config_t* config) {
    ...
    client->status.recipient_count = client->recipient_count;
...
void email_client_format_body(email_client_t* client, const notification_event_t* event, 
    ...
    if (client->config.html_format) {
        ...
        const char* src = event->message;
        char* dst = html_message;
        size_t remaining = sizeof(html_message) - 1;
...
static char* create_email_message(email_client_t* client, const char* recipient, 
                                 const char* subject, const char* body) {
    if (!client || !recipient || !subject || !body) return NULL;
    ...
    snprintf(message, message_size,
             ...
             "\r\n"
             "%s",
             strlen(client->config.from_name) > 0 ? client->config.from_name : "autonomy",
