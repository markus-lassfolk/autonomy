                       ...
                       struct blob_attr *msg)
{
    ...
    blobmsg_add_f32(&bb, "current_lat", g_state.current_lat);
    blobmsg_add_f32(&bb, "current_lon", g_state.current_lon);
    blobmsg_add_f32(&bb, "current_accuracy", g_state.current_accuracy);
    blobmsg_add_u32(&bb, "current_confidence", g_state.current_confidence);
    blobmsg_add_f32(&bb, "gps_health_score", g_state.gps_health_score);
                        ...
                        struct blob_attr *msg)
{
    ...
    for (int i = 0; i < g_state.gps_source_count; i++) {
        ...
        blobmsg_add_f32(&bb, "lat", g_state.gps_sources[i].lat);
        blobmsg_add_f32(&bb, "lon", g_state.gps_sources[i].lon);
        blobmsg_add_f32(&bb, "accuracy", g_state.gps_sources[i].accuracy);
