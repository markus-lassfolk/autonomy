...
echo "=== Installing Example API on RUTOS Device (Overlay Method) ==="

# Create directories in writable overlay
echo "Creating directories in overlay filesystem..."
...
chmod +x /overlay/root/upper/usr/libexec/rpcd/example.lua
...
# Create symbolic links to make files accessible in expected locations
...
if ubus list | grep -q example; then
    echo "✓ Example API is available"
else
    echo "✗ Example API not found"
fi

echo "Testing API calls..."
if ubus call example status > /dev/null 2>&1; then
    echo "✓ API calls working"
else
    echo "✗ API calls failed"
fi
