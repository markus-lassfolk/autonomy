# PowerShell script to deploy VUCI package using direct installation
...
Write-Host "=== Deploying VUCI Package via Direct Installation ===" -ForegroundColor Green

# Step 1: Transfer the installation script
Write-Host "Step 1: Transferring installation script..." -ForegroundColor Yellow
$scpCommand = "scp -i `"$SSHKey`" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null `"install-example-api.sh`" ${DeviceUser}@${DeviceIP}:/tmp/"
...
$sshCommand = "ssh -i `"$SSHKey`" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null ${DeviceUser}@${DeviceIP} 'chmod +x /tmp/install-example-api.sh && /tmp/install-example-api.sh'"
...
Write-Host "VUCI package has been deployed via direct installation!" -ForegroundColor Green
Write-Host ""
