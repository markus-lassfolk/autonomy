# Comprehensive VUCI Package Builder and Deployer
# Based on working packages: ntpd, upnp
# Includes thorough verification at each step

$DeviceIP = "192.168.80.1"
$DeviceUser = "root"
$SSHKey = "C:\Users\markusla\OneDrive\IT\RUTOS Keys\rusos_private_key_openssh"

Write-Host "=== Building and Deploying Proper VUCI Packages ===" -ForegroundColor Green
Write-Host "Based on working package patterns: ntpd, upnp" -ForegroundColor Cyan
Write-Host ""

# Step 1: Build packages using WSL
Write-Host "Step 1: Building packages with comprehensive verification..." -ForegroundColor Yellow
wsl bash build-proper-vuci-packages.sh
...
if ($apiPackage -and $uiPackage) {
    ...
    Write-Host "  ✅ Packages built with SDK build system" -ForegroundColor Green
