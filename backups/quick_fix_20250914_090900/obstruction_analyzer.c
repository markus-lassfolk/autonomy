...
// Initialize obstruction analyzer configuration with defaults
void obstruction_analyzer_config_init_defaults(obstruction_analysis_config_t *config) {
    if (!config) return;
    
    config->snr_threshold = OBSTRUCTION_SNR_THRESHOLD;
    config->min_elevation = MIN_ELEVATION_DEGREES;
    config->max_elevation = MAX_ELEVATION_DEGREES;
    config->use_adaptive_threshold = false;
    config->adaptive_threshold_factor = 0.8;
    config->smoothing_window_size = 3;
}
...
void obstruction_analyzer_dish_to_absolute_coords(
    ...
    // Note: We rotate by (/2 - elevation) to align with horizon
